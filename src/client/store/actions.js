...
export default {
  saveRateToDb({ commit, state }, data) {
    return new Promise(resolve =>
      db.settings.get(1).then(result => {
        if (result !== undefined) {
          db.settings.update(1, { isBtcRate: data }).then(() => resolve())
        } else {
          db.settings.put({ id: 1, isBtcRate: data }).then(() => resolve())
        }
      })
    )
  },
  getRateFromDb({ commit, state }) {
    return new Promise(resolve =>
      db.settings.get(1).then(result => {
        if (result === undefined) {
          resolve(undefined)
        } else {
          resolve(result.isBtcRate)
        }
      })
    )
  },
  saveCoinToDb({ commit, state }) {
    return new Promise(resolve =>
      db.settings.get(1).then(result => {
        if (result !== undefined) {
          db.settings
            .update(1, {
              shortName: state.coin.shortName,
  ...
  getSelectedRangeFromDbAction({ commit, state }, chartName) {
    return new Promise(resolve =>
      db.charts.get(chartName).then(result => {
        if (result === undefined) {
          resolve(undefined)
        } else {
          // console.log(
