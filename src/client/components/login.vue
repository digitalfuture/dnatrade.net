<template>
  <section id='login' class='top-2'>
    ...
    <section v-if='!isError'>
      <transition name='fade'>
        <md-button v-if='isValidating' class='md-icon-button lavender cloud animation-pulse' aria-label='Loading indicator'>
          <md-icon>cloud_queue</md-icon>
        </md-button>

        <md-button v-else v-show='isLogging' class='md-icon-button lavender cloud animation-pulse' aria-label='Loading indicator'>
          <md-icon>cloud_queue</md-icon>
        </md-button>
      </transition>
    </section>
    ...
    <transition name='fade'>
      <div id='social-container' v-if='!isLogged && !isLogging && !isValidating'>
        <md-button class='md-icon-button social-button' href='/auth/google' aria-label='Log in with Google Plus'>
          <i class='glow social fab fa-google-plus-square'></i>
        ...
        <!-- <md-button class='md-icon-button social-button' @click.passive='authenticate("facebook")' aria-label='Log in with Facebook'>
         ...
         -->
        <md-button class='md-icon-button social-button' href='/auth/twitter' aria-label='Log in with Twitter'>
          <i class='glow social fab fa-twitter-square'></i>
