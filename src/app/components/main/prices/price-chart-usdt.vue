  ...
  <md-card md-with-hover class='item large-margin-right'>
      ...
      <md-layout md-gutter md-vertical-align='start'>
        ...
        <md-layout md-column md-gutter>
          ...
          <h3 class='md-subhead hand-write dimmed'>
            {{ coin.shortName }} / USD exchange rate
          </h3>
        </md-layout>
      </md-layout>
    </md-card-header>
    ...
    <md-card-media>
      <transition name='fade'>
        <div class='image'>
          <div id='price-chart-usdt' alt="Price chart in USDT"></div>
        </div>
      </transition>
    </md-card-media>
  </md-card>
...
export default {
  name: 'price-chart-usdt',
  computed: {
    ...mapState(['coin', 'charts', 'balances', 'prices']),
    ...mapState(['fullTime']),
    ...mapGetters(['priceChartUsdtData'])
  },
  watch: {
    coin: {
      handler() {
  ...
  methods: {
    ...
    ...mapActions(['updatePriceChartUsdtDataAction', 'updateChartAction']),
    getCurrencyIcon(name) {
      return getIcon(name)
    },
    chartHandler() {
      this.updatePriceChartUsdtDataAction()
        .then(() =>
          this.updateChartAction({
            chartName: 'priceChartUsdt',
            chartElem: this.plotPriceChart(this.priceChartUsdtData)
          })
        )
        .catch(e => {
          this.showError()
    ...
    plotPriceChart(data) {
      ...
      const chartOptions = {
        ...
        rangeSelector: {
          selected: this.charts.priceChartUsdt.selectedRange,
          inputEnabled: false,
                  ...
                  switch (e.rangeSelectorButton.type) {
                    case 'month': {
                      if (e.rangeSelectorButton.count == 1)
                        this.charts.priceChartUsdt.selectedRange = 0
                      if (e.rangeSelectorButton.count == 3)
                        this.charts.priceChartUsdt.selectedRange = 1
                      if (e.rangeSelectorButton.count == 6)
                        this.charts.priceChartUsdt.selectedRange = 2

                      return
                    }
                    case 'all': {
                      this.charts.priceChartUsdt.selectedRange = 3
                    }
                  }
                }
              }
      ...
      return new StockChart('price-chart-usdt', chartOptions)
    }
  },
  mounted() {
    this.chartHandler()
