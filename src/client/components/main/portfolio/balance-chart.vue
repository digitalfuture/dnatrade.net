...
export default {
  ...
  methods: {
    ...mapMutations(['showError', 'switchRate', 'switchPortfolio']),
    ...mapActions(['updateChartAction', 'updateBalanceChartDataAction']),
    ...
    plotBalanceChart(data) {
      ...
      const chartOptions = {
        ...
        rangeSelector: {
          selected: this.charts.balanceChart.selectedRange,
          inputEnabled: false,
                  ...
                  switch (e.rangeSelectorButton.type) {
                    case 'month': {
                      if (e.rangeSelectorButton.count == 1)
                        this.charts.balanceChart.selectedRange = 0
                      if (e.rangeSelectorButton.count == 3)
                        this.charts.balanceChart.selectedRange = 1
                      if (e.rangeSelectorButton.count == 6)
                        this.charts.balanceChart.selectedRange = 2

                      return
                    }
                    case 'all': {
                      this.charts.balanceChart.selectedRange = 3
                    }
                  }
                }
              }
